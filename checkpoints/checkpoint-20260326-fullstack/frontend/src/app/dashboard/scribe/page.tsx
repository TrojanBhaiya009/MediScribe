...
export function ScribeComponent(props: { isDemo?: boolean, onDemoComplete?: () => void, onDemoStart?: () => void }) {
    ...
    return (
        <div className={styles.dashboardContent} style={{ padding: '24px 32px' }}>
            ...
            <div className={styles.workflowCard} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 24px', marginBottom: '8px', borderLeft: isRecording ? '4px solid #ef4444' : currentCache?.status === 'approved' ? '4px solid #8b5cf6' : '4px solid var(--color-accent-green)', overflow: 'visible' }}>
                ...
                <div ref={emrContainerRef} style={{ display: 'flex', flexDirection: 'column', gap: '16px', height: '100%', overflowY: 'auto', paddingRight: '8px' }}>
                    ...
                    {emr ? (
                        <>
                            ...
                            {/* Chief Complaint & HPI */}
                            <ConfidenceCard title="Chief Complaint & HPI" confidence={emr.chiefComplaint?.confidence || 'blank'} fieldKey="chiefComplaint" onConfirm={handleConfirmField} confirmed={isConfirmed('chiefComplaint')} onEdit={handleEditField}>
                                <div>
                                    <strong style={{ color: 'var(--color-text-primary)' }}>CC:</strong>{' '}
                                    <span style={{ color: 'var(--color-text-primary)', cursor: 'text' }} onDoubleClick={() => handleEditField('chiefComplaint', prompt('Edit Chief Complaint:', emr.chiefComplaint?.value || '') || emr.chiefComplaint?.value || '')}>{emr.chiefComplaint?.value || 'N/A'}</span>
                                    <br />
                                    <strong style={{ color: 'var(--color-text-primary)', marginTop: '4px', display: 'inline-block' }}>HPI:</strong>{' '}
                                    <span style={{ color: 'var(--color-text-primary)', cursor: 'text' }} onDoubleClick={() => handleEditField('hpi', prompt('Edit HPI:', emr.hpi?.value || '') || emr.hpi?.value || '')}>{emr.hpi?.value || 'N/A'}</span>
                                </div>
                            </ConfidenceCard>

                            {/* Diagnosis */}
                            <ConfidenceCard title="Provisional Diagnosis" confidence={emr.diagnosis?.confidence || 'blank'} fieldKey="diagnosis" value={emr.diagnosis?.value} onConfirm={handleConfirmField} confirmed={isConfirmed('diagnosis')} onEdit={handleEditField} />

                            {/* Safety Check — Agent 2 Output */}
                            <div className={styles.workflowCard} style={{
                                padding: '20px',
                                borderLeft: emr.safetyCheck?.overallSafetyStatus === 'critical_flags' ? '4px solid #ef4444' : emr.safetyCheck?.overallSafetyStatus === 'warnings_present' ? '4px solid #f59e0b' : '4px solid var(--color-accent-green)',
                            }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                                    ...
                                    <span style={{
                                        ...
                                        background: emr.safetyCheck?.overallSafetyStatus === 'safe' ? 'rgba(16, 185, 129, 0.1)' : emr.safetyCheck?.overallSafetyStatus === 'critical_flags' ? 'rgba(239, 68, 68, 0.1)' : 'rgba(245, 158, 11, 0.1)',
                                        color: emr.safetyCheck?.overallSafetyStatus === 'safe' ? 'var(--color-accent-green)' : emr.safetyCheck?.overallSafetyStatus === 'critical_flags' ? '#ef4444' : '#f59e0b',
                                    }}>
                                        {emr.safetyCheck?.overallSafetyStatus === 'safe' ? '✅ SAFE' : emr.safetyCheck?.overallSafetyStatus === 'critical_flags' ? '🚨 CRITICAL' : '⚠️ WARNINGS'}
                                    </span>
                                </div>
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                            {emr.medications.map((med, idx) => (
                                                <div key={idx} style={{
                                                    ...
                                                    background: CONFIDENCE_COLORS[med.confidence || 'green'].bg,
                                                    borderLeft: `3px solid ${CONFIDENCE_COLORS[med.confidence || 'green'].border}`,
                                                }}>
                                                    ...
                                                    <ConfidenceBadge confidence={med.confidence || 'green'} fieldKey={`med_${idx}`} onConfirm={handleConfirmField} confirmed={isConfirmed(`med_${idx}`)} />
                                                </div>
                                            ))}
                                        </div>
                                    ) : (
                                    ...
                                    {emr.investigations?.length ? (
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                            {emr.investigations.map((inv, idx) => (
                                                <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 10px', borderRadius: '6px', background: CONFIDENCE_COLORS[inv.confidence || 'green'].bg }}>
                                                    ...
                                                    <ConfidenceBadge confidence={inv.confidence || 'green'} fieldKey={`inv_${idx}`} onConfirm={handleConfirmField} confirmed={isConfirmed(`inv_${idx}`)} />
                                                </div>
                                            ))}
                                        </div>
                                    ) : (
                            ...
                            {/* Plan + Disease Risk */}
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                                <ConfidenceCard title="Treatment Plan" confidence={emr.plan?.confidence || 'blank'} fieldKey="plan" value={emr.plan?.value} onConfirm={handleConfirmField} confirmed={isConfirmed('plan')} onEdit={handleEditField}>
                                    <div>
