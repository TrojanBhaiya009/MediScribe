        ...
        <div className={styles.dashboardContent} style={{ padding: '24px 32px', height: '100vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '260px minmax(350px, 1fr) minmax(300px, 400px)', gap: '24px', flex: 1, alignItems: 'stretch', minHeight: 0 }}>
                ...
                <div className={styles.workflowCard} style={{ padding: 0, display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
                    ...
                    <div style={{ flex: 1, overflowY: 'auto' }}>
                        ...
                        {pharmacyPatients.length === 0 ? (
                            ...
                            <div key={p.id} onClick={() => setActivePatientId(p.id)} style={{ padding: '16px', borderBottom: '1px solid rgba(255,255,255,0.05)', cursor: 'pointer', background: activePatientId === p.id ? 'rgba(128, 103, 68, 0.1)' : 'transparent', borderLeft: activePatientId === p.id ? '3px solid #806744' : '3px solid transparent' }}>
                ...
                <div style={{ background: '#ffffff', borderRadius: '4px', padding: '40px', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)', display: 'flex', flexDirection: 'column', color: '#0f172a', borderTop: '8px solid var(--color-navy-blue)', height: '100%', minHeight: '600px', overflowY: 'auto' }}>
                    ...
                    <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '6px', marginBottom: '24px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                        ...
                        <div>
                            ...
                            <div style={{ fontSize: '14px', fontWeight: 'bold', color: activePatient?.status === 'Pharmacy' ? '#806744' : '#16a34a' }}>{activePatient?.status || '—'}</div>
                        </div>
                    </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    ...
                    {hindiSummary ? (
                        <div className={styles.workflowCard} style={{ padding: '24px', borderTop: '6px solid #806744', display: 'flex', flexDirection: 'column', flex: 1 }}>
                            <div style={{ textAlign: 'center', marginBottom: '20px' }}>
                                <span style={{ fontSize: '36px' }}>🇮🇳</span>
                                <h2 style={{ fontSize: '20px', color: 'var(--color-text-primary)', margin: '8px 0 4px 0' }}>Patient Summary (Hindi)</h2>
                                <p style={{ fontSize: '13px', color: '#806744', margin: 0 }}>Generated by Agent 3 — Hindi Summarizer</p>
                            </div>

                            <div style={{ background: 'rgba(128, 103, 68, 0.05)', border: '1px solid rgba(128, 103, 68, 0.2)', padding: '16px', borderRadius: '12px', marginBottom: '16px', textAlign: 'center' }}>
                            ...
                            <div style={{ flex: 1 }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                    {hindiSummary.patientSummary?.medicationInstructions?.map((med, idx) => (
                                        <div key={idx} style={{ background: 'rgba(128, 103, 68, 0.05)', borderLeft: '4px solid #806744', padding: '14px', borderRadius: '6px' }}>
                                ...
                                {hindiSummary.patientSummary?.followUpNote && (
                                    <div style={{ marginTop: '16px', padding: '12px', background: 'rgba(111, 120, 83, 0.08)', borderRadius: '8px', fontSize: '13px', color: '#6f7853' }}>
                                        📅 {hindiSummary.patientSummary.followUpNote}
                                    </div>
                                )}
                            </div>
